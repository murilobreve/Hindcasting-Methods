...
reg <- do.call(rbind, r_p_v_p)

clust <- do.call(rbind, r_p_v)

lat_lon_RMSE <- list()
for (j in seq_along(variables_all)) {
    lat_lon_RMSE[[j]] <-
        cbind(ll_norfolk[ll_norfolk[, 1]  %in%  toupper(str_sub(r_p_v_p[[j]][, 5], 1, 5)), ], r_p_v_p[[j]], variables_all[j])
...
for (j in seq_along(variables_all)) {
    lat_lon_RMSE[[j]] <-
        cbind(ll_norfolk[ll_norfolk[, 1]  %in%  toupper(str_sub(r_p_v[[j]][, 5], 1, 5)), ], r_p_v[[j]], variables_all[j])
    colnames(lat_lon_RMSE[[j]])[9] <- "Variable"
    colnames(lat_lon_RMSE[[j]])[3] <- "lon"
}
...
d.acRMSE <- as.numeric(as.character(d.ac$RMSE))
...
Methods <- c(rep("PLSR", length(d.a[,1])), rep("ClustAnEn", length(d.ac[,1])))
...
ggmap(chicago) +
    ...
    ggtitle('PLSR vs PCClust predicting with one variable')  +   scale_color_gradient(low = "blue",
...
ggsave("permap_norfolk.png", dpi = 300, scale = 1.5)
